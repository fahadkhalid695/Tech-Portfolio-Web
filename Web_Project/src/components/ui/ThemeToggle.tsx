/**
 * ThemeToggle — BB8 droid toggle, converted from styled-components to
 * plain CSS injected via a <style> tag (no extra dependencies).
 *
 * Language: TypeScript React (.tsx)
 * - Uses the existing useTheme() hook (ThemeContext)
 * - Checked state = DARK mode (night sky / stars visible)
 * - Unchecked state = LIGHT mode (day sky / clouds visible)
 * - Size controlled by --toggle-size CSS variable (set to 10px here for the nav)
 * - Accessible: aria-label on the wrapping label
 *
 * Original design credit: CSS BB8 toggle (community CSS art)
 * Adapted for React + TypeScript, no styled-components.
 */
...
const STYLE_ID = 'bb8-toggle-styles';
