      ...
      <div className="container-custom">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <motion.div 
              className="space-y-6 text-light/80 leading-relaxed"
              initial={{ opacity: 0, y: 20 }}
              animate={inView ? { opacity: 1, y: 0 } : {}}
            ...
            >
              <h4 className="text-lg font-semibold text-light mb-3">Areas of Interest</h4>
              <div className="grid grid-cols-2 gap-3">
                {interests.map((interest, index) => (
                  <motion.div
                    key={interest.name}
                    className="flex items-center gap-3 p-3 rounded-lg bg-dark/30 border border-dark-light/30 hover:border-primary-500/30 transition-all duration-300"
                    whileHover={{ 
                      scale: 1.05, 
                  ...
                  >
                    <div className={`text-${interest.color}-400`}>
                      {interest.icon}
                    </div>
                    <span className="text-sm font-medium text-light/90">{interest.name}</span>
                  </motion.div>
                ))}
              </div>
            </motion.div>
            ...
            >
              {stats.map((stat, index) => (
                <motion.div
                  key={stat.label}
                  className="text-center p-4 rounded-xl border border-dark-light/30 transition-all duration-300 relative overflow-hidden"
                  style={{
                    background: 'rgba(255, 255, 255, 0.05)',
                  ...
                  whileHover={{ 
                    y: -5,
                    borderColor: `rgba(${stat.color === 'primary' ? '0, 255, 240' : stat.color === 'success' ? '16, 185, 129' : '110, 68, 255'}, 0.5)`,
                    boxShadow: `0 20px 40px rgba(${stat.color === 'primary' ? '0, 255, 240' : stat.color === 'success' ? '16, 185, 129' : '110, 68, 255'}, 0.2)`,
                  }}
                ...
                >
                  ...
                  <motion.div
                    ...
                    style={{
                      background: `linear-gradient(135deg, rgba(${stat.color === 'primary' ? '0, 255, 240' : stat.color === 'success' ? '16, 185, 129' : '110, 68, 255'}, 0.1), transparent)`,
                    }}
                    animate={{
                  ...
                  <div className="text-xs text-light/60 relative z-10">{stat.label}</div>
                </motion.div>
              ))}
            </motion.div>
